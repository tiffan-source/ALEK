...
function CodeTravail({affaire, handleCritereSelect, rict}) {
    ...
    useEffect(() => {
        if(affaire)
            (async ()=>{
                let {data} = await axios.get(process.env.REACT_APP_STARTURIBACK  + `/get_critere_about_code_travail/${affaire}/`);
  ...
  return (
    <div>
        ...
        <h2 className='p-4 shadow-lg bg-gray-200 text-gray-700'>Code du travail</h2>
        <div className=''>
