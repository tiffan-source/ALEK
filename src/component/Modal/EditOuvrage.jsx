...
function EditOuvrage({handleClose, ouvrageAffaire}) {
    ...
    useEffect(()=>{
      (async()=>{
		setNewName(ouvrageAffaire.ouvrage.libelle)
        setLoad(false)
      })();
    }, []);

    let editOuvrage = async()=>{
        try {
			if (newName) {
				await axios.put(process.env.REACT_APP_STARTURIBACK + `/admin/affaireouvrage/${ouvrageAffaire.id}/`,{
                    ...ouvrageAffaire,
                    rename : newName
                });
			}
    ...
    <div id="defaultModal" tabIndex="-1" aria-hidden="true" className="fixed top-0 left-0 right-0 z-50 w-full overflow-x-hidden overflow-y-auto h-full flex justify-center items-center bg-[#000a]">
      {success && <Flash type={"success"} setFlash={setSuccess}>Ouvrage editer</Flash> }
      ...
      <div className="relative w-full max-w-4xl max-h-full">
        <div className="relative bg-gray-300 rounded-lg shadow ">
          ...
          <div className="px-6 space-y-6">
            {!load ? <LabelInput label={"Nom de l'ouvrage"} value={newName} onChange={(e)=>{
          ...
          <div className="flex items-center justify-between p-6 space-x-2 border-t border-gray-200 rounded-b ">
            {!action ? <Button action={()=>{
                setAction(true)
                editOuvrage()
            }}>Editer</Button> : <span className='text-green-600'>Ouvrage en cours d'edition</span> }
